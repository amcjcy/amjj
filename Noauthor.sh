 ...
 * @returns {Promise<unknown>}
 */
async function sendNotify(text, desp, params = {}, author = 'noauthor') {
  //提供6种通知
...
echo "$lrr" | sed -e "s/noauthor/$1/g" > /root/jd/scripts/sendNotify.js

echo "$(date "+%Y-%m-%d %H:%M:%S") sendNotify 替换完成 !"
